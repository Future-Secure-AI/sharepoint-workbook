...
import openWorkbookByPath from "./openWorkbookByPath";
...
describe("openWorkbookByPath", () => {
	it("can read workbook by path", async () => {
		...
		const readHandle = await openWorkbookByPath(driveRef, itemPath);
	...
	it("throws for missing file", async () => {
		const driveRef = getDefaultDriveRef();
		const itemPath = driveItemPath(generateTempFileName("xlsx"));
		await expect(openWorkbookByPath(driveRef, itemPath)).rejects.toThrow();
	});
});
